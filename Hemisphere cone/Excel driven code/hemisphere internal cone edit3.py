...
f.write('CALL SetPress Q35 P1; Try dropping back to 28\n')
...
list2 = array of horizontal moves between each concentric layer
...
"""
for list1, list2, list3 in zip(L[1:], DIN[1:], DUP[1:]):  
    counter = counter + 1
    counter4 = 0 
#    f.write(';LAYER = {}\n'.format(counter))
#    f.write(';DIN  = {}\n'.format(list2))
...
#                if counter == 30:
#                    print('counter4 = ', counter4, 'nonzerolist length = ', len(nonzerolist1))
#                    print(list2[i-1])
                elif 1 <= counter4 < len(nonzerolist1)-1:
                    if list2[i-1] <= 0:
                        break
                    #perim_diff_within_layer = round(list1[i] - list1[i+1],3)  
                    f.write('G1 X-{} Y{} Z0.00 F{}; MOVE INWARD\n'.format(round(list2[i-1],3),
                            ext_width,speed))

                else:
                    f.write(';MARKER\n')
                    f.write('G1 X-{} Y{} Z0.00 F{}; CIRCULAR ALIGNMENT\n'.format(round(list2[i-1],3),
                            ext_width,speed))
...
#                    print(last_perimeter)
                    ...
                    f.write('G1 X{} Y{} Z0.00 F{};        OUTWARD MOVE \n'
                            .format(round(list2[i-1],3),ext_width,speed))

                    f.write('G3 X0.00 Y-{} I-{} J-{} F{}; CIRCULAR MOTION \n'
                            .format(ext_width,round(new_reverse_list1[i],3),half_width,speed))
                    print('layer = ', counter, 'perimeter = ', counter2)

                else:
                    f.write(';MARKER2\n')
                    #perim_diff_within_layer = round(list1[i]-list1[i+1],3)                    
                    f.write('G1 X{} Y{} Z0.00 F{};        CIRCULAR ALIGNMENT \n'
                            .format(perim_diff_within_layer,ext_width,speed))
                    f.write('G3 X0.00 Y-{} I-{} J-{} F{}; CIRCULAR MOTION \n'
                            .format(ext_width,round(new_reverse_list1[i],3),half_width,speed))
                    counter2 = 0
