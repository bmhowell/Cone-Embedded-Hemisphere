...
By far the most complicated part of the code. Don't mess with this part.
...
speed = 15                                                                      # mm/s 
ext_width = 0.838                    #0.5842                                                              # mm (nozzle width)
...
inner_radius = np.ones(len(inner_cone)) * round(inner_cone[-1], 4)              # inner radius for a solid hemisphere

hemi_cone_diff = np.round(outer_hemisphere-inner_radius,2)

tot_perims_layer = tot_perimeters_layer(hemi_cone_diff)

perimeters = perimeter_arrays(inner_radius, outer_hemisphere, tot_perims_layer)
...
f = open("solid_hemisphere_inner_green_tip.txt","w+")
...
for list1, list2, list3 in zip(L[1:],DIN[1:],DUP):                             
    ...
    f.write('G1 X0.00 Y{} Z{} F{}; MOVE UP LAYER\n'.format(threshold,       # for all other steps, move up in the z direction
          z_height,speed))
...
#            print('reverse_list1 = {}'.format(reverse_list1))
            ...
            if counter4 < 1: 
#                f.write('G1 X-{} Y0.00 Z0.00 F{}; \n'                          #for solid cone, remove the inward motion. Th internal pipe requires not x,y adjustment
#                        .format(din_cone,speed))
    ...
    counter2 = 0 
    counter4 = 0 
hemi_hemi_outer_radius = 15 + half_width
x_move = round(hemi_hemi_outer_radius - round(reverse_list1[0],3),4)
f.write('CALL TogglePress P1; Turn pressure box off \n')
f.write('G1 Z5.00 F50; \n')
f.write('G1 X{} F50; \n'.format(x_move + 0.019))
#f.write('G1 Y{} F50; \n'.format(-threshold/2))
f.close()
import os
b1 = 'solid_hemisphere_inner_green_tip.txt'
